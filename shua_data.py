...
t_map = {'model_name': '型号', 'manufacture': '生产企业', 'brand': '品牌', 'vehicle_type': '车辆类型',
         ...
         'fast_charging_ratio': '快充倍率', 'wheelbase': '轴距', 'power_type': '动力类型', 'drive_range': '续驶里程（km，总）'
         }
...
def prepare_sql(table):
    ...
    insert_sql = 'insert ignore %s (%s) values ' \
                 ...
                 '%%s,%%s,%%s,%%s,%%s,%%s,%%s,%%s,%%s,%%s,%%s,%%s,%%s,%%s,%%s,%%s); ' % (table, insert_keys)  # 38个占位符
    return insert_sql
...
def insert_mysql(pmonth='201904'):
